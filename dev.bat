@echo off
setlocal
REM always run from the folder this script is in (project root)
set ROOT=%~dp0
cd /d "%ROOT%"
...
IF "%CMD%"=="proto" (
  protoc -I proto --go_out=proto --go_opt=paths=source_relative --go-grpc_out=proto --go-grpc_opt=paths=source_relative proto/streams.proto
  goto :eof
)
...
IF "%CMD%"=="run-all" (
  ...
  timeout /t 1 >nul
  start cmd /k "cd /d %ROOT% && set GRPC_TARGET=localhost:50051 && go run .\tools\sim\cmd\sim"
  goto :eof
)
